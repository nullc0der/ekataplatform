...
function store(name, val) {
    if (typeof (Storage) !== "undefined") {
      ...
      $.ajax({
        type:  'POST',
        async: false,
        data: {'uistate': get('uistate')},
        url: '/' + lang + '/profile/setui/'
      });
    } else {
      window.alert('Please use a modern browser to properly use this website!');
    }
  }
/**
...
function _init() {
      ...
      $(document).on('click', toggleBtn, function (e) {
        ...
        if ($(window).width() > (screenSizes.sm - 1)) {
          if ($("body").hasClass('sidebar-collapse')) {
            ...
            $("#userimage").removeClass('collapsed-avatar');
          } else {
            $("body").addClass('sidebar-collapse').trigger('collapsed.pushMenu');
            $("#userimage").addClass('collapsed-avatar');
          }
        }
  ...
  $.AdminLTE.boxWidget = {
    ...
    collapse: function (element) {
      ...
      if (!box.hasClass("collapsed-box")) {
        ...
        box_content.slideDown(_this.animationSpeed, function () {
          ...
          uistate[col_id].collapsed = false;
          store('uistate', JSON.stringify(uistate));
        });
      }
    },
    remove: function (element) {
      ...
      var uistate = JSON.parse(get('uistate'));
      if (uistate[col_id]) {
        uistate[col_id].removed = true;
        store('uistate', JSON.stringify(uistate));
      }
      col.hide();
      if (col_id === 'timerwidget') {
        $("#nrel").show();
      }
      if (col_id === 'connectionwidget') {
        $("#conn").show();
      }
      if (col_id === 'profilewidget') {
        $("#profc").show();
      }
    }
  };
